...
function ExtractMask(seg, color)
  ...
  elseif color == 'blue2' then
    mask = torch.lt(seg[1], 0.1)
    mask:cmul(torch.cmul(torch.gt(seg[2], 0.33-0.1), torch.lt(seg[2], 0.33+0.1)))
    mask:cmul(torch.gt(seg[3], 1-0.1))
  elseif color == 'lightgreen' then
    mask = torch.cmul(torch.gt(seg[1], 0.66-0.1), torch.lt(seg[1], 0.66+0.1))
    mask:cmul(torch.gt(seg[2], 1-0.1))
    mask:cmul(torch.cmul(torch.gt(seg[3], 0.33-0.1), torch.lt(seg[3], 0.33+0.1)))
